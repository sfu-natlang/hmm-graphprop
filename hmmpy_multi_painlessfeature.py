...
class HMM:
    ...
    def __init__( self, n_states=1, **args ):
        ...
        :Keywords:
          ...
          - `V` - list of all observable symbols and their features as list of [word, feature1, feature2, feature3, ..., feature d]
          - `A` - transition matrix
...
def baum_welch( hmm, Obs_seqs, **args ): 
    ...
    normUpdate=verbose=validating=painless_feature= 0
    ...
    if 'painlessFeature' in args: painless_feature = args['painlessFeature']
    ...
    for epoch in xrange( epochs ):
        ...
        # ( observable symbol probability distribution )
            ...
            if painless_feature:
                print "Start using logstic regression for smoothing"



                # training a logistic regression
                # for each hidden varibale we train a logistic regression 
		# labels are expected counts 
                for hidden in xrange(hmm.N):
                    labels = Expect_si_vk_all[hidden,:]
                    #starting_w = ????
                    # new logistic regression with LBFGS
                    #logistic_regression_train(features,labels,random_start)

                    result = logistic_regression_predict()
                    for v,word in enumerate(hmm.V):
                        Expect_si_vk_all [ hidden,v ] = result[v]
