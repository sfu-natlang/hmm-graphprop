...
def compute_expectations(data,hmm):
...
def baum_welch( hmm, Obs_seqs, **args ): 
    ...
    if 'graphprop' in args: graph_prop=1
   
    graph_prop = 1 
    K = len( Obs_seqs ) # number of observation sequences
    start = time.time() # start training timer
    sc = SparkContext("local","Parallel HMM")
    #bupdatePi = sc.broadcast(updatePi)
    #bupdateA = sc.broadcast(updateA)
    #bupdateB = sc.broadcast(updateB)
    #bscaling = sc.broadcast(scaling)
    #bgraph = sc.broadcast(graph)
    #bnormUpdate = sc.broadcast(normUpdate)
    #bverbose = sc.broadcast(verbose)
    #bvalidating = sc.broadcast(validating)
    #bgraphprob = sc.broadcast(graphprop)
    ...
    for epoch in xrange( epochs ):
        ...
        #LL_epoch =  sc.accumulator(0)
        #LL_epoch = sc.broadcast(LL_epoach)
        ...
        logger.addHandler(logging.StreamHandler())
        #logger.info(">>>>><<<<<<<<<<<<<<<<<<")
        temp = dist_Obs_seqs.flatMap(lambda a: compute_expectations(a,hmm)).reduceByKey(lambda a,b: a+b)#.keys().collect()
        #updatePi = temp.filter(lambda a,b: if a )

        #.reduceByKey(lambda a,b: merge(a,b)).count()#.collect()#reduceByKey(lambda a,b: merge(a,b))#.keys().saveAsTextFile('temp_spark4')#cache()#count().saveAsTextFile('temp_spark3') #reduceByKey(lambda a,b: a+b)
        #logger.info(keys)
        
        params = temp.collect()  #filter(lambda keyValue: keyValue == 'Expect_si_vk_all').collect()#.count()#.reduceByKey(lambda a,b: a+b).collect()
        Expect_si_vk_all = params[0][1]
        Expect_si_all_TM1 = params[1][1]
        Expect_si_t0_all = params[2][1]
        Expect_si_sj_all = params[3][1]
        Expect_si_sj_all_TM1 = params[4][1]
        Expect_si_all = params[5][1]


        #logger.info(params)
        #logger.info(Expect_si_t0_all)
        ...
        #Expect_si_t0_all = temp.filter(lambda keyValue: keyValue == "Expect_si_t0_all").reduceByKey(lambda a,b: a+b).collect()
        #Expect_si_t0_all = Expect_si_t0_all / numpy.sum( Expect_si_t0_all )
        #hmm.Pi = Expect_si_t0_all
        ...
        if validating:
            ...
            if epoch > 0:
                ...
                best_epoch = 0
        if verbose:
            #logger.info("Finished epoch %d in %d secs" % ( epoch+1, int( time.time() - start_epoch ) ), LL_epoch)
...
def dishonest_casino_test( graph = True ):
    ...
    c.append( Obs )
    #c.append(Obs2)
    baum_welch( hmm, [c], epochs=15, verbose=1)
###################################################################################
###################################################################################
###################################################################################
