"""
Multi-thread python code for HMM training and decoding. 
The core of this code is addopted from a code by Michael Hamilton. Then it upgraded to use multi-threading and some useful features  by Ramtin Seraj. 
...
def compute_expectations(data,hmm,epoch,process_number=0,normUpdate=0,updateB=1,scaling=1):
    ...
    for Obs in data:
        if ow > 0 and ow % 100 == 0:
            print "epoch %d: %d seqs processed by one of the processors" % ( epoch+1, ow )
        ow += 1
        Obs = list( Obs )
    ...
    results = []
    results.append(("Expect_si_vk_all",Expect_si_vk_all))
    results.append(("Expect_si_t0_all",Expect_si_t0_all))
    results.append(("Expect_si_all",Expect_si_all))
    results.append(("Expect_si_all_TM1",Expect_si_all_TM1))
    results.append(("Expect_si_sj_all",Expect_si_sj_all))
    results.append(("Expect_si_sj_all_TM1",Expect_si_sj_all_TM1))

    print  "One of processes is done !"
    return results
...
def baum_welch( hmm, Obs_seqs, **args ): 
    ...
    :Keywords:
      ...
      - `graphprop` - flag to apply graph propagation as a regularizer.
      - `numberOfProcesses` number of processors to use. default is 4. 
    """
    # Setup keywords
    ...
    normUpdate=verbose=validating=graphprop= 0
    ...
    if 'graphprop' in args: graphprop = args['graphprop']
    if 'numberOfProcesses' in args:  number_of_process = args['numberOfProcesses']
    ...
    P_L = int(K/number_of_process)

    #dist_Obs_seqs = sc.parallelize(Obs_seqs,4).cache() 
    dist_Obs_seqs = [Obs_seqs[i*P_L:(i+1)*P_L] for i in xrange(number_of_process-1) ]+[Obs_seqs[(number_of_process-1)*P_L:]]
    ...
    for epoch in xrange( epochs ):
        ...
        #res = parmap(lambda x:compute_expectations(x,hmm),dist_Obs_seqs)

        pipe=[Pipe() for x in dist_Obs_seqs ]
        proc = []
        index = 1
        for x,(p,c) in izip(dist_Obs_seqs,pipe):
            print "process %d started"%index
            proc.append(Process(target=spawn(lambda x:compute_expectations(x,hmm,epoch,normUpdate,updateB)),args=(c,x)))
            index +=1
        #proc=[Process(target=spawn(lambda x:compute_expectations(x,hmm,epoch,index+1,normUpdate,updateB)),args=(c,x)) for  index,x,(p,c) in izip(count(),dist_Obs_seqs,pipe)]
        ...
        #print res
        for p in xrange(number_of_process):
            Expect_si_vk_all = Expect_si_vk_all + res[p][0][1]
            Expect_si_t0_all =  Expect_si_t0_all + res[p][1][1]
            Expect_si_all  = numpy.add(Expect_si_all, res[p][2][1])
            Expect_si_all_TM1 = Expect_si_all_TM1 + res[p][3][1]
            Expect_si_sj_all =  Expect_si_sj_all +  res[p][4][1]
            Expect_si_sj_all_TM1 = Expect_si_sj_all_TM1 + res[p][5][1]
        ...
        # ( observable symbol probability distribution )
            Expect_vi_all = numpy.zeros( [ hmm.M ] ,float)  
            #for i in xrange( hmm.N ):
            #    Expect_si_vk_all[ i,: ] = Expect_si_vk_all [ i,: ] / Expect_si_all[ i ]
            
            for v in  xrange(hmm.M) :
                Expect_vi_all[v] = sum(Expect_si_vk_all [ :,v ])

            if graphprop:
                ##### writing to file ######
                # should I change transition probs too ??
                with open("expected_emission_pro_inp",'w') as outp:
                        #print hmm.V.shape
                    for v,word in enumerate(hmm.V):
                        outp.write(word)
                        for count in xrange(hmm.N):
                            if Expect_vi_all[v] == 0 :
                                outp.write("\t"+ str(1.0 /float( hmm.N )))
                            else:
                                outp.write("\t"+ str(Expect_si_vk_all [ count ,v ]/Expect_vi_all[v]))
                        # each row shows for each V what are the distribution of labels
                        outp.write("\n")
                print "Emission info are written to the file."
                print "Propagation done!"

                # Graph_Propagate_mu_nu_params.o
                #args = ("/cs/natlang-projects/users/Golnar/Documents/Academic/Code/Graph-Subramanya/Graph_Propagation/Graph_Propagate_NoParams.o")
		args =["/cs/natlang-projects/users/Golnar/Documents/Academic/Code/Graph-Subramanya/Graph_Propagation/Graph_Propagate_mu_nu_params.o","0","0.5","0.00000001"]
                ...
                # do not change the order of vocab and states 
                with open("expected_emission_pro_out",'r') as inp:
                    i = 0
                    for line in inp:
                        row = line.strip().split()[1:]
                        for j in xrange(hmm.N):
                            value = float(row[j]) * float(Expect_vi_all[i])
                            Expect_si_vk_all [ j,i ] = value
                        i += 1
...
def dishonest_casino_test( graph = True ):
    ...
    #c.append(Obs2)
    baum_welch( hmm, c, epochs=15,graph = 0, verbose=1)
###################################################################################
###################################################################################
###################################################################################
